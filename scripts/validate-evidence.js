...
export function validateEvidenceBundle({ readEvidence, writebackEvidence, policy }) {
  ...
  if (recomputedContext) {
    try {
      ...
      if (sha256(recomputedArtifacts) !== sha256(certifiedArtifacts)) errors.push("writeback: generated artifacts do not match deterministic recomputation.");
    } catch (error) {
  ...
  for (const [claim, state] of [
    ...
    ["Downstream impact paths traced", "PASS"],
