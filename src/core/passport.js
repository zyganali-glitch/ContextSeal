...
export function buildPassport(run, approval, now = new Date()) {
  ...
  const body = {
    ...
    policy: { version: run.policyVersion, sha256: run.policyHash },
...
export function verifyPassport(run, policy, now = new Date()) {
  ...
  if (passport.policy?.version !== policy.policyVersion || passport.policy?.sha256 !== sha256(policy)) {
  ...
  if (!Number.isFinite(observedAt) || contextAgeMs > policy.contextMaxAgeHours * 3_600_000 || contextAgeMs < -5 * 60_000) {
  ...
  if (run.mode === "datahub") {
    ...
    if (!binding || sha256(binding) !== sha256(liveEvidenceBinding(run))) errors.push("passport does not bind the complete live evidence summary and provenance");
    ...
    if (binding?.observedAt !== run.context.observedAt || live?.observedAt !== run.context.observedAt) errors.push("live evidence timestamp does not match normalized context");
    if (run.context?.evidenceBoundary !== "LIVE_DATAHUB_MCP_NORMALIZED") errors.push("run context is not verified live MCP normalization");
    if (run.context?.provenance?.rawEvidenceHash !== live?.rawEvidenceHash) errors.push("normalized context provenance does not bind raw MCP evidence");
    if (live && Array.isArray(live.evidence)) {
      try {
    ...
    const contextEvidence = run.evidence?.find((item) => item.claim === "DataHub context retrieved");
    if (contextEvidence?.state !== "PASS") errors.push("DataHub context evidence is not PASS");
