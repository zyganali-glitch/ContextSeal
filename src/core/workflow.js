...
export function analyzeChange({ request: rawRequest, context, policy, mode = "fixture", now = new Date() }) {
  ...
  return {
    ...
    evidence: [
      ...
      { claim: "Downstream impact paths traced", state: impactState, artifact: impactState === "FIXTURE" ? "fixture impact paths" : "impact paths" },
...
export function decideRun(run, rawApproval, now = new Date()) {
  ...
  const manifest = buildArtifactManifest(run, passport);
