...
export function analyzeChange({
  ...
  const impactState = mode === "datahub"
    ? (liveContextVerified ? (impact.counts.total ? "PASS" : "WARN") : "NOT_RUN")
    : "FIXTURE";
  ...
  return {
    ...
    evidence: [
      ...
      {
        claim: "Entity-level downstream impact traced",
        state: impactState,
        artifact: impactState === "PASS" || impactState === "WARN" ? `${impact.counts.total} assets across exact MCP paths`
...
export function decideRun(run, rawApproval, now = new Date()) {
  ...
  const evidence = run.evidence.map((item) =>
    item.claim === "Human scope approval recorded"
      ? { ...item, state: "PASS", artifact: approval.reviewer }
      : item);
  const decided = {
    ...run,
    state: approval.decision === "APPROVE" ? "APPROVED_FOR_WRITEBACK" : "REJECTED",
    approval: { ...approval, decidedAt: now.toISOString() },
    evidence
  };
  return {
    ...decided,
    passport: buildPassport(decided, approval, now)
  };
}
