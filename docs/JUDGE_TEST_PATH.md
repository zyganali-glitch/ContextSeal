...
- Both artifacts are synthetic-local, never production evidence; `npm run evidence:check` passed for this recorded export, and the final release SHA must include this committed proof bundle and pass the same integrity check. A fresh final-stage recapture is optional and must be recorded with separate provenance if taken.
