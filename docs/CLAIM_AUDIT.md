...
| DataHub write-back inheritance | `downgrade` | The mutation implementation is real and test-covered, but the committed export predates the current provenance, verify-then-skip, and exactly-one-marker contract and is therefore `STALE`. |
...
| `docs/DEVPOST_SUBMISSION.md` | Challenge category drift and entity-type overstatement risk | `downgrade` | Primary category remains aligned to Metadata-Aware Code Generation; recorded read proof retains the typed downstream summary, while write-back is labeled `STALE` until recapture. |
...
| `docs/JUDGING_MAP.md` | Live proof could be read as fully typed cross-entity lineage or current write-back | `downgrade` | Preserves the typed seeded read summary while labeling the companion write-back export `STALE` and preserving the separate live/fixture boundary. |
...
| `examples/outputs/live-datahub-writeback-evidence.json` | Historical export lacks the current proof provenance and idempotency receipt shape | `downgrade` | Marked `STALE`; require a new synthetic-local capture, second idempotent rerun, export, and passing `npm run evidence:check`. |
