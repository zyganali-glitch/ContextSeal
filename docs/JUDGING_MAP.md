...
- The recorded live-local read artifact preserves the five bounded read-only MCP tool types and a typed downstream summary with six `DATASET`, two `DATA_JOB`, and two `DASHBOARD` entities across seeded platforms. Its companion write-back export is `STALE` until a new provenance-bound, verify-then-skip capture passes `npm run evidence:check` on synthetic metadata.
